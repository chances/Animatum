...
namespace Animatum.Updater.Downloader
{
    ...
    public class FileDownloader
    {
        ...
        public int CurrentURLIndex { get; private set; }
        ...
        public FileDownloader(List<string> urls, string downloadfolder, bool downloadAll = false)
        {
            urlList = urls;
        ...
        void bw_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            if (DownloadAll)
            {
                ...
                {
                    long size = DownloadData.GetFileSize(s);
                    if (size > -1)
            ...
            foreach (string s in urlList)
            {
                CurrentURLIndex = urlList.IndexOf(s);
                ...
                if (!DownloadAll && (ex == null || bw.CancellationPending))
                {
                    allFailedWaitingForResponse = false;
                    break;
                }
        ...
        void BeginDownload()
        {
            ...
            try
            {
                ...
                while ((readCount = data.DownloadStream.Read(buffer, 0, BufferSize)) > 0)
                {
                    ...
                    if (DownloadAll)
                        totalBytesDownloaded += readCount;
                    ...
                    if (DownloadAll)
                        calculateBps(totalBytesDownloaded, totalDownloadSize);
                    else
                    ...
                    if (!bw.CancellationPending)
                    {
                        if (DownloadAll)
                        {
                            int percentDone = (int)((totalBytesDownloaded * 100) / totalDownloadSize);
            ...
            catch (Exception e)
            {
                if (string.IsNullOrEmpty(DownloadingTo))
                    throw new Exception(string.Format("Error trying to save file: {0}\n\n{1}", e.Message, e.StackTrace), e);
                else
        ...
        void calculateBps(long BytesReceived, long TotalBytes)
        {
            ...
            int secondsRemaining = (int)((TotalBytes - BytesReceived) / bps);

            //downloadSpeed = Path.GetFileName(urlList[CurrentURLIndex]) + " - " + downloadSpeed;
            if (secondsRemaining  == 1)
                downloadSpeed += "  " + secondsRemaining.ToString() + " second remaining";
            else
                downloadSpeed += "  " + secondsRemaining.ToString() + " seconds remaining";
    ...
    class DownloadData
    {
        ...
        static WebRequest GetRequest(string url)
        {
            ...
            if (request is HttpWebRequest)
            {
                ...
                ((HttpWebRequest)request).UserAgent = "Mozilla/5.0 (Windows; U; MSIE 9.0; Windows NT 6.1; en-US; Animatum Updater)";
            }
